<nav class="navbar navbar-expand-lg bg-dark border-bottom border-bottom-dark ticky-top bg-body-tertiary"
    ...
    <div class="container">
        <a class="navbar-brand fw-light" href="/"><span class="fas fa-brain me-1">
            </span>{{ config('app.name') }} {{-- Using the config section to call the app name --}}</a>
        ...
        <div class="collapse navbar-collapse justify-content-end" id="navbarNav">
            <ul class="navbar-nav">
                ...
                @endif
                    <li class="nav-item df">
                        <a class="{{ Route::is('users.show') ? 'active' : '' }} nav-link" href={{ route('users.show', Auth::user()->id) }}>{{ Auth::user()->name }}</a>
                    </li>
                    <li class="nav-item">
